...
module.exports = ikyy = async (ikyy, mek) => {
	try {
        ...
        //FAKEREPLY TROLI
            const ftroli = {
                  key: {fromMe: false,participant: `0@s.whatsapp.net`, ...(from ? { remoteJid: "16505434800@s.whatsapp.net" } : {})},message: {orderMessage: {itemCount : 2021,status: 1,surface : 1,message: `Created by FoxyCodeX`,orderTitle: 'Bang',thumbnail: fs.readFileSync('./media/Nakano.jpg'), sellerJid: '0@s.whatsapp.net'}}}
...
📅 𝐃𝐚𝐭𝐞: *${moment().utcOffset('+0700').format('DD/MM/YYYY')}*
...
┷┯ ☾ OTHER MENU ☽
   ...
   ╰╼❥ © FOXY-BOT ❣️`
               buttons = [{buttonId: `${prefix}owner`,buttonText:{displayText: '👑 OWNER 👑'},type:1},{buttonId:`${prefix}rules`,buttonText:{displayText:'🛑 RULES 🛑'},type:1},{buttonId:`${prefix}donasi`,buttonText:{displayText:'💰 DONASI 💰'},type:1}]
               ...
               buttonsMessage = {
               contentText: `${menu}`,
               footerText: '© FoxyCodeX\nDon\'t forget to Donate 🤗', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}

               prep = await ikyy.prepareMessageFromContent(from,{buttonsMessage},{quoted: mek, contextInfo: { mentionedJid: [sender]}})
...
case 'creator':
  ...
  await sleep(1000)
  txtt =`Hai Kak *${pushname}*\nItu Ownerku, ada keperluan apa ya?`
...
donasiTxt = `*${ucapanWaktu} @${sender.split('@')[0]}* 👋
Mau Donasi? Silahkan Scan QR Code diatas 🙌
...
buttons = [{buttonId: `${prefix}menu`,buttonText:{displayText: '📌 MENU 📌'},type:1}]
               ...
               buttonsMessage = {
               contentText: `${donasiTxt}`,
               footerText: '© FoxyCodeX • 2021', imageMessage: imageMsg,
               buttons: buttons,
               headerType: 4
}
...
//------------------< G R U P >-------------------
      ...
      case 'demote':
       ...
       case 'setgrupname':
             ...
             reply(`*Berhasil Mengganti Nama Group*`)
              break
       case 'setdesc':
              if (!isGroup) return reply(mess.only.group)
             ...
             reply(`*Berhasil Mengganti Deskripsi Group*`)
              break
       case 'setppgrup':
              if (!isGroup) return reply(mess.only.group)
              if (!isBotGroupAdmins) return reply(mess.only.Badmin)
             ...
             reply(`*Berhasil Mengganti Profile Group*`)
              } else {
       ...
       case 'afk': 
         if (!isGroup) return reply(mess.only.group)
              if (isAfkOn) return reply('*Kamu sudah AFK sebelumnya!*')
              const reason = q ? q : '-'
       ...
       case 'groupinfo':
              ...
              break
       case 'tagall':
              if (!isGroupAdmins && !isOwner) return reply('*Luh Siapa Bjier*\n*Atmin Bukan, Owner Bukan :v*')
              if (!isGroup) return reply(mess.only.group)
              let arr = [];
              let messTagall = body.slice(8)
              let txti = `*[ TAG ALL ]*\n\n✉️ *Message:*\n${messTagall}\n\n`
...
case 'caripesan':  //by ANU TEAM
       ...
       case 'bugreport':
              ...
              ikyy.sendMessage('6288286421519@s.whatsapp.net',`*Bug Report:* ${teks}`, text)
              break
       case 'readall':
...
//------------------< Menunya Bang:v >-------------------
      case 'infoig':
             reply(`Youtube FaxBotz\n\nGrup : https://chat.whatsapp.com/GlAuhF2Sh8n4blusjELDrL`)
             break
      case 'Fax':
